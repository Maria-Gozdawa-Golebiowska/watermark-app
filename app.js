...
const startApp = async () => {
  ...
  if (options.watermarkType === 'Text watermark') {
    ...
    if (fs.existsSync(inputImagePath)) {
      addTextWatermarkToImage(inputImagePath, './img/' + prepareOutputFilename(options.inputImage), options.watermarkText);
    } else {
      console.error('Something went wrong... Try again.');
    }
  } else {
    ...
    if (fs.existsSync(inputImagePath) && fs.existsSync(watermarkImagePath)) {
      addImageWatermarkToImage(inputImagePath, './img/' + prepareOutputFilename(options.inputImage), watermarkImagePath);
    } else {
      console.error('Something went wrong... Try again.');
    }
  }
